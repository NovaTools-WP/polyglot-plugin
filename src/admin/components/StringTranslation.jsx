...
export default function StringTranslation() {
	...
	const [newString, setNewString] = useState({ domain: "", name: "", value: "" });
	...
	const handleRegisterString = async () => {
		...
		try {
			await api.post("/strings", {
				...
				name: newString.name || newString.value.toLowerCase().replace(/\s+/g, "_"),
	...
	if (error) return <ErrorState message={error} onRetry={() => { fetchMeta(); fetchStrings(domainFilter); }} />;
						...
						<tbody>
							{filteredStrings.map((str) => (
								<tr key={str.id} className="border-b last:border-b-0">
									...
									<td className="px-4 py-3 font-medium text-gray-900">{str.value}</td>
									{languages.map((lang) => {
			...
			{showRegisterForm && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
					<div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
						<h2 className="mb-4 text-lg font-semibold text-gray-900">Register String</h2>
