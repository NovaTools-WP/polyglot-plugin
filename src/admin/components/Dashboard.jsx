...
import { LoadingSpinner, ErrorState, ProgressBar, StatCard, PolyglotNav } from "@/admin/components/shared";
...
export default function Dashboard() {
	...
	const fetchData = async () => {
		...
		try {
			...
			if (settingsData?.translation_api) {
				...
				const count = [api_keys.deepl_key, api_keys.google_key, api_keys.openai_key].filter(Boolean).length;
	...
	const avgContentPercentage = activeLanguages.length > 0
		? Math.round(activeLanguages.reduce((sum, l) => sum + (l.content_percentage || 0), 0) / activeLanguages.length)
		: 0;
	const avgStringsPercentage = activeLanguages.length > 0
		? Math.round(activeLanguages.reduce((sum, l) => sum + (l.strings_percentage || 0), 0) / activeLanguages.length)
		: 0;

	return (
		<div className="p-6">
			<PolyglotNav />
			<h1 className="mb-6 text-2xl font-bold text-gray-900">Polyglot Dashboard</h1>
			...
			<div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<StatCard
					title="API Providers"
					value={providerCount}
					subtitle={providerCount > 0 ? "Configured" : "No providers configured"}
				/>
			</div>

			<div className="rounded-lg border bg-white shadow-sm">
				<div className="border-b px-4 py-3">
					<h2 className="text-lg font-semibold text-gray-900">Language Status</h2>
				</div>
